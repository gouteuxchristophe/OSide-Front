...
import { resetUserErrorUpdate, resetSuccessUpdate, updateUser, getUserById, setTemporalyUserUpdate, resetTemporalyUserUpdate } from "../../store/reducers/user";
...
export default function ModalUpdateContent({ closeModal }: { closeModal: () => void }) {
  ...
  const temporalyUserUpdate = useAppSelector(state => state.user.temporalyUserUpdate)
  ...
  useEffect(() => {
    ...
    if (updateFirstName !== '' || updateLastName !== '' || updateEmail !== '' || updateBio !== '' || updatePassword !== '' || updateConfirmPassword !== '' || technoTemporaly.length > 0) {
      dispatch(setTemporalyUserUpdate({
        id: user.id,
        ...
        passwordConfirm: updateConfirmPassword,
        ability: technoTemporaly
      }))
    }
  }, [updateFirstName, updateLastName, updateEmail, updateBio, updatePassword, updateConfirmPassword])
          ...
          <div className="px-10 pt-2 sm:mt-[2rem] rounded relative">
            <form onSubmit={handleUpdateUserSubmit} className="flex flex-col items-center bg-primary0 p-5 rounded relative">
              ...
              <div className="mb-2 sm:mb-6 flex flex-col sm:flex-row items-center justify-center sm:gap-5 w-full">
                ...
                <input onChange={(e) => setUpdateFirstName(e.currentTarget.value)} defaultValue={temporalyUserUpdate.first_name || user.first_name} type="text" className="shadow-sm text-sm rounded block p-2.5 sm:w-[40%]" placeholder="prénom" />
              </div>
              <div className="mb-2 sm:mb-6 flex flex-col sm:flex-row items-center justify-center sm:gap-5 w-full">
                <label htmlFor="lastname" className="text-center sm:text-left block text-sm font-medium sm:w-[10%] sm:pb-0 pb-2">Nom :</label>
                <input onChange={(e) => setUpdateLastName(e.currentTarget.value)} defaultValue={temporalyUserUpdate.last_name || user.last_name} type="text" className="shadow-sm text-sm rounded block p-2.5 sm:w-[40%]" placeholder="nom" />
              </div>
              <div className="mb-2 sm:mb-6 sm:flex sm:items-center sm:justify-center sm:gap-5 sm:w-full">
                ...
                <input onChange={(e) => setUpdateEmail(e.currentTarget.value)} defaultValue={temporalyUserUpdate.email || user.email} type="email" className="shadow-sm text-sm rounded block w-full p-2.5 sm:w-[40%]" placeholder="name@flowbite.com" required />
              </div>
              <div className="mb-2 sm:mb-6 sm:flex sm:items-center sm:justify-center sm:gap-5 sm:w-full">
              ...
              <div className="mb-2 sm:mb-6 sm:flex sm:items-center sm:justify-center sm:gap-5 sm:w-full">
                ...
                <textarea onChange={(e) => setUpdateBio(e.currentTarget.value)} defaultValue={temporalyUserUpdate.bio || user.bio} className="shadow-sm text-sm rounded block w-full p-2.5 sm:w-[40%]" placeholder="Décrivez vous" />
              </div>
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {technoTemporaly.map((techno) => (
                    <div className="relative" key={`${techno.id}-${techno.label}`}>
                      <span className={`p-2 rounded border border-solid border-${techno.color}`}>{techno.label}</span>
                    </div>
                  ))}
                </div>
              </div>
